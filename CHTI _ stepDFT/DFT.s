...
DFT_ModuleAuCarre proc
	mov R3,#0 ; R3 : Compteur n
	push {R4-R8}
	MOV R4,#0 ;R4:Partie Reel
	MOV R5,#0 ;R5:Partie Imaginaire
	LDR R2,=TabCos
	LDR R7,=TabSin
	...
	;R7 : Valeur du sin
boucle
	LDRH R6,[R0,R3,LSL #1 ];R6 : Signal
	MUL R12, R1, R3 ; k*n
	AND R12, R12, #0x3F ;Modulo pour éviter de sortir du tableau
	push {R12}
	
	;Partie réelle
	LDRSH R8,[R2,R12,LSL #1];on charge la valeur TabCos[k*n]
	MUL R12,R6,R8
	ADD R4,R4,R12; Xr+=*(x+n)*TabCos[k]
	
	
	
	;Partie Imaginaire
	pop {R12}
	LDRSH R8,[R7,R12,LSL #1];on charge la valeur TabSin[k]
	MUL R12, R6, R8
	ADD R5,R5,R12;Xi+=*(x+n)*TabSin[k]
